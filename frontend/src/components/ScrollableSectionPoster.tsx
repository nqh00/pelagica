...
interface ScrollableSectionPosterProps extends Omit<
...
const ScrollableSectionPoster = forwardRef<HTMLAnchorElement, ScrollableSectionPosterProps>(
    (
        {
            item,
            posterUrl,
        ...
        const isSquareType =
            !forceLandscape &&
            (item?.Type === 'Playlist' || item?.Type === 'MusicAlbum' || isArtist);
        ...
        if (posterFailed) {
            return (
                <Link ref={ref} to={linkTo} className={className} {...rest}>
                    <div
                    ...
                    <p
                        className={`mt-2 text-sm line-clamp-1 text-ellipsis break-all ${maxTextWidth}`}
                    >
                        {itemName || item?.Name || ''}
                    </p>
                    {children}
        ...
        return (
            <Link ref={ref} to={linkTo} className={className} {...rest}>
                ...
                >
                    ...
                    <Skeleton
                        className={`absolute bottom-0 left-0 right-0 ${skeletonClasses} -z-1`}
                    />
                    {isDirectPlay && (
...
ScrollableSectionPoster.displayName = 'ScrollableSectionPoster';
